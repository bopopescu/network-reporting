...
(function($){
  ...
  $(document).ready(function() {
    ...
    function setupDashboardStatsChart() {
      ...
      this.trafficChart = new Highcharts.Chart({
        ...
        yAxis: {
          labels: {
            formatter: function() {
              var text = Highcharts.numberFormat(this.value, 0) + ' ms';
              return text;
            }
          }
        },
        tooltip: {
          formatter: function() {
            ...
            value = Highcharts.numberFormat(this.y, 0) + ' ms latency';
    ...
    // set up buttons
    $('#dashboard-status-detailsButton')
      .button({ icons: { primary: "ui-icon-triangle-2-n-s" } })
      .click(function(e) {
        e.preventDefault();
        if ($('#dashboard-status-details').is(':visible'))
          $('#dashboard-status-details').slideUp('fast');
        else
          $('#dashboard-status-details').slideDown('fast');
      });
