	...
	$.fn.dataTable = function( oInit )
	{
		...
		function _fnSort ( oSettings, bApplyClasses )
		{
				...
				(oSettings.aaSorting.length !== 0 || oSettings.aaSortingFixed !== null) )
			{
                ...
                //all tree data groups are sorted
                treeData.l0.sort(sortFunction);
                $.each(treeData.l1, function(k, v) {
                    v.sort(sortFunction);
                });
                $.each(treeData.l2, function(k, v) {
                    v.sort(sortFunction);
                });
                //create a copy of the top level array
                var fin = treeData.l0.slice();
                $.each(treeData.l1, function(k, v) {
                    var ins_at = fin.indexOf(treeData.mapper[k]);
                    fin = array_splice(ins_at, fin, v);
                });
                $.each(treeData.l2, function(k, v) {
                    var ins_at = fin.indexOf(treeData.mapper[k]);
                    fin = array_splice(ins_at, fin, v);
                });
				oSettings.aiDisplayMaster = fin.slice();
			}
        ...
        /* *********************
         *
         * Create sorting "groups" with a link to where they go
         * they are organized by level so we know how to insert them 
         * into the larger picture. Since I backreference with ids and this
         * (retarded) plugin uses list indices into the aoData array I have a mapper
         * which maps from ids to indices.  Huzzah
         *
         * ********************/
        function treeData(settings) {
            ...
            for (var idx = 0; idx < data.length; idx++) {
                var id = data[idx].nTr.id;
                mapper[id] = idx;
                var classNames = data[idx].nTr.className.split(' ');
                var prnt, level;
                if (classNames[0] == 'level0') {
                    level0.push(idx);
                }
                else if (classNames[1] != 'level0') {
                    prnt = classNames[0];
                    if (classNames[1] == 'level1') {
                        if (level1[prnt]) {
                            level1[prnt].push(idx);
                        }
                        else {
                            level1[prnt] = [idx];
                        }
            ...
            return {'l0' : level0, 'l1': level1, 'l2': level2, 'mapper': mapper};
        }
