...
class StatsModel(db.Expando):
    ...
    req = db.IntegerProperty(default=0)
    imp = db.IntegerProperty(default=0)
    clk = db.IntegerProperty(default=0)
    conv = db.IntegerProperty(default=0)


    # uniq user counts
    # user_count, request_user_count, impression_user_count,
    # impression_user_count, click_user_count
    usr = db.IntegerProperty(default=0) # NOTE: name change
    req_usr = db.IntegerProperty(default=0)
    imp_usr = db.IntegerProperty(default=0)
    clk_usr = db.IntegerProperty(default=0)

    # List of requests, useful for debugging
    reqs = db.ListProperty(str,indexed=False)

    # total revenue (cost)
    rev = db.FloatProperty(default=float(0))
    ...
    def __init__(self, parent=None, key_name=None, ad_network_stats=None, **kwargs):
        ...
        # REFACTOR: standardize naming to abbreviated versions
        AD_NETWORK_STATS = {'revenue': 'rev',
                            'attempts': 'req',
                            'impressions': 'imp',
                            'clicks': 'clk'}
        if ad_network_stats:
    ...
    def __eq__(self, other):
        return self._account == other._account \
           ...
           and self.req == other.req \
           and self.imp == other.imp \
           and self.clk == other.clk \
    ...
    def __add__(self,s):
        ...
        attributes = dict(parent=self.parent_key() or s.parent_key(),
                          ...
                          imp=self.imp + s.imp,
                          clk=self.clk + s.clk,
                          conv=self.conv + s.conv,
                          rev=self.rev + s.rev,
                          usr=max(self.usr, s.usr), # TODO: takes the maximum user count over the period (high water mark)
                          req_usr=self.req_usr,
                          imp_usr=self.imp_usr,
                          clk_usr=self.clk_usr,
from google.appengine.api.datastore_types import Key
...
offline=%s, %s,%s,%s,%s)" % (self.date or self.date_hour,
    ...
    def get_cpm(self):
        ...
        if self.imp > 0:
            return self.rev * 1000 / float(self.imp)
        else:
            return 0
    ...
    def cpc(self):
        if self.clk > 0:
            return self.rev / float(self.clk)
        else:
            return 0
    ...
    def cpa(self):
        if self.conv > 0:
            return self.rev / float(self.conv)
        else:
            return 0
    ...
    def _dict_properties(self):
        ...
        pseudo_props = ['att', 'cpa', 'cpc', 'cpm', 'fill_rate', \
...
class SiteStats(db.Model):
  ...
  imp = db.IntegerProperty(default=0)
  clk = db.IntegerProperty(default=0)
  usr = db.IntegerProperty(default=0)

  # total revenue (cost)
  rev = db.FloatProperty(default=float(0))

  # conversion information
  conv = db.IntegerProperty()
  ...
  def fill_rate(self):
    return self.imp / float(self.req)

  def ctr(self):
    if self.imp > 0:
      return self.clk / float(self.imp)

  def conv_rate(self):
    if self.clk > 0 and self.conv > 0:
      return self.conv / float(self.clk)
    else:
      return None
...
# ( useful for cool map shit )
  ...
  def add_clk_with_rev(self, revenue):
    self.clk += 1
    self.rev += rev
    self.put()
  ...
  def add_usr(self,u):
    if hasattr(self,'unique_user_set'):
      ...
      self.unique_usr = len(self.unique_user_set)
    else:
  ...
  def __add__(self, s):
    return SiteStats(site=self.site,
      ...
      clk = self.clk + s.clk,
      rev = self.rev + s.rev,
      unique_usr = self.unique_usr + s.unique_usr, # TODO: we need to de-dupe this!
      conv = self.conv + s.conv if self.conv and s.conv else None )

  def __repr__(self):
    from django.utils import simplejson
    self._geo_requests_json = simplejson.dumps(getattr(self,'_geo_request_dict',{}))
    try:
      return "SiteStats{site=%s, owner=%s(%s), R=%d/I=%d/C=%d/R=%.2f, users=%d, geo=%s}" % \
              (self.site.key() if self.site else "None", self.owner.__class__.__name__ if \
              self.owner else '',self.owner.key() if self.owner else "None", self.req, \
                      self.imp, self.clk, self.rev, self.unique_usr,self._geo_req_json)
    except:
