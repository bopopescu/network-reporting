...
class AdHandler(webapp.RequestHandler):
	def get(self):
		...
		if c and c.e_cpm() >= h["backfill_threshold_cpm"]:
			# ok show our ad
			logging.info("eCPM exceeded threshold, showing ad")
			...
			logging.info("eCPM did not exceed threshold, failing")
			html = None
		else:
			# never mind, we should use a backfill strategy
			logging.info("eCPM did not exceed threshold, using backfill: %s" % h["backfill"])
	...
	def get_creatives(c, h, request, q, rgeocode, format):
		...
		logging.info("keywords=%s site_key=%s" % (keywords, site_key))
		...
		logging.info("geo_predicates=%s, device_predicates=%s, format_predicates=%s" % (geo_predicates, device_predicates, format_predicates))
		...
		if len(ad_groups) > 0:
			...
			logging.info("returned %d creatives, winning eCPM=%.2f" % (len(creatives), winning_ecpm))
			return creatives
		else:
			return []
