...
class MobFoxServerSide(ServerSide):
    ...
    def __init__(self,request,adunit=None,*args,**kwargs):
        self.html_params = {}
        self.pub_id = adunit.account.mobfox_pub_id or 'ed28510f1b4fb866f1c20181a08ae04f'
    ...
    def _add_extra_headers(self):
        """
        add extra headers to the post but cannot escape the brackets (e.g. h[])
        hence we can't just use the generic paylod
        output looks something like h[foo]=bar&h[foo2]=bar2
        """
        ...
        return '&'.join(['h[%s]=%s'%(h,v) for h,v in  headers])
    ...
    def parse_xml(self,document):
        ...
        if ad_type == "imageAd":
            ...
            for param in image_params:
                value = self._getText(dom.getElementsByTagName(param)[0])
