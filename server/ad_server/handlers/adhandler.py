...
class AdHandler(webapp.RequestHandler):
    ...
    def get(self):
        ...
        if (not experimental and rand_dec < experimental_fraction):
            ...
            # return self._redirect_to_experimental()
        ...
        if c:
            user_adgroup_daily_key = frequency_capping.memcache_key_for_date(udid,now,c.ad_group.key())
            user_adgroup_hourly_key = frequency_capping.memcache_key_for_hour(udid,now,c.ad_group.key())
            ...
            ad_click_url = "http://%s/m/aclk?id=%s&cid=%s&c=%s&req=%s&udid=%s&appid=%s" % (self.request.host, adunit_id, c.key(), c.key(),request_id, udid, appid)
            # ad an impression tracker URL
            track_url = "http://%s/m/imp?id=%s&cid=%s&udid=%s&appid=%s" % (self.request.host, adunit_id, c.key(), udid, appid)
