...
echo N47935 | appcfg.py download_data --application=mopub-inc --url=http://mopub-inc.appspot.com/remote_api --filename=datastore7 --email=olp@mopub.com --passin --kind=MarketPlaceConfig
echo $DESTINATION_PASS | appcfg.py upload_data --email=olp@mopub.com --filename=datastore7 --application=$DESTINATION_APP --url=$DESTINATION_URL --passin      
