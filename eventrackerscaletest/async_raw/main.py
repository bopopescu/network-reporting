...
LOG_KEY_FORMAT = 'k:%(account_name)s:%(log_index)02d'
INDEX_KEY_FORMAT = 'k:%(account_name)s:index'
HEAD_KEY_FORMAT = 'k:%(account_name)s:head'
TAIL_KEY_FORMAT = 'k:%(account_name)s:tail'
...
def log(**kwargs):
    ...
    log_key = LOG_KEY_FORMAT%dict(account_name=account_name,log_index=log_index)
    ...
    try:
        t = Task(name=task_name,params={'account_name':account_name},countdown=TIME_BUCKET,method='GET')
