...
def threadproc():
    ...
    while not quitevent.is_set():
        try:
            ...
            uid = uuid.uuid4()
            adunits = ['ahRldmVudHJhY2tlcnNjYWxldGVzdHILCxIEU2l0ZRiJJww', 'ahRldmVudHJhY2tlcnNjYWxldGVzdHILCxIEU2l0ZRipRgw']
            creatives = ['ahRldmVudHJhY2tlcnNjYWxldGVzdHIPCxIIQ3JlYXRpdmUYoh8M','ahRldmVudHJhY2tlcnNjYWxldGVzdHIPCxIIQ3JlYXRpdmUYkU4M','ahRldmVudHJhY2tlcnNjYWxldGVzdHIPCxIIQ3JlYXRpdmUY-VUM']
            appids =['angrybirds1','angrybirds2','angrybirds3']
            ...
            req_cnt += 1
            ce_url = "http://%s/m/aclk?udid=%s&appid=%s&id=%s&cid=%s&reqcnt=%s&inst=%s&test=%s"%(HOST,uid,appid,adunit,creative,req_cnt,INSTANCE_ID,TEST_ID)
            aoe_url = "http://%s/m/open?v=1&udid=%s&id=%s&reqcnt=%s&inst=%s&test=%s"%(HOST,uid,appid,req_cnt,INSTANCE_ID,TEST_ID)
            req_cnt_lock.release()
            
            fail_lock.acquire()
            print
            print "ClickEvent URL: ", ce_url
            print "AppOpenEvent URL: ", aoe_url
            print
            fail_lock.release()
            
            resp, content = h.request(ce_url)
            resp, content = h.request(aoe_url)

            if 'ConversionLogged:True' in content:
