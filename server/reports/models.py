...
class Report(db.Model):
    ...
    """
    def parse_report_blob(self, blobreader, dimkey_to_obj):
        ...
        for line in blobreader:
            ...
            keys = keys.split(':')
            vals = eval(vals)
            req, att = self.get_stats_info(keys, dimkey_to_obj)
            #I'm using list comprehension for you Nafis
            bid_infos = [self.get_bid_info(idx, key, dimkey_to_obj) for idx, key in enumerate(keys)]
            keys = [self.get_key_name(idx, key, dimkey_to_obj) for idx, key in enumerate(keys)]
