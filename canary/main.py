...
class PerformanceHandler(webapp.RequestHandler):
    def get(self):
        ...
        averages = [(d, m) for d, m, r in aggregate_requests]
        last = [r for d, m, r in averages]
...
class PingHandler(webapp.RequestHandler):
    def get(self):
        ...
        for ad_test in ad_tests:
            logging.info("Pinging " + ad_test.ad_id)
        ...
        latency_sum = 0
        for rpc in rpcs:
            latency_sum += json.loads(rpc.get_result())['request_ms']
        aggregate_requests = memcache.get("aggregate_requests") or []
        
        # pick a random request 
        request_dict = json.loads(rpcs[int(random.random() * len(URLS))].get_result())
        now = datetime.datetime.now()
        r = Request(host=request_dict['host'], urls=request_dict['url'], success=request_dict['success'], status_code=request_dict['status_code'],
                    status_message=request_dict['status_message'], request_ms=request_dict['request_ms'], response_size=request_dict['response_size'],
                    t=now)
        aggregate_requests.insert(0, (now, latency_sum / len(ad_tests), r))
...
application = webapp.WSGIApplication([('/performance', PerformanceHandler), # shows a splash page containing aggregate performance data
