...
def website_pending(request,*args,**kwargs):
  send_mail("New User","%s has signed up for an account. Someone please activate him if necessary. https://appengine.google.com/datastore/explorer?submitted=1&app_id=mopub-inc&viewby=gql&query=SELECT+*+FROM+Account+where+active+%3D+False%0D%0A&namespace=&options=Run+Query"%request.user.email,'olp@mopub.com',['beta@mopub.com'],fail_silently=True)
